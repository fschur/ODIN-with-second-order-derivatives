...
# Libraries
from odin.utils.kernels_auto import *
...
class GaussianProcess(object):
    ...
    def __init__(self,
        ...
        setting).
        :param use_sec_grads: boolean, indicates whether to use second order derivatives.
        """
        ...
        self.jitter = tf.constant(1e-6, dtype=tf.float64)
        ...
        if self.use_sec_grads:
            self.diff_diff_c_phi_matrices = None
            self.c_phi_diff_diff_matrices = None
            self.diff_diff_c_phi_diff_matrices = None
            self.diff_c_phi_diff_diff_matrices = None
            self.diff_diff_c_phi_diff_diff_matrices = None
        return
    ...
    def build_supporting_covariance_matrices(self, t: tf.Tensor,
        ...
        if self.use_sec_grads:
            self.diff_diff_c_phi_matrices =\
                self._build_diff_diff_c_phi_matrices(t_new)
            self.c_phi_diff_diff_matrices =\
                self._build_c_phi_diff_diff_matrices(t_new)
            self.diff_diff_c_phi_diff_matrices =\
                self._build_diff_diff_c_phi_diff_matrices(t_new)
            self.diff_c_phi_diff_diff_matrices =\
                self._build_diff_c_phi_diff_diff_matrices(t_new)
            self.diff_diff_c_phi_diff_diff_matrices =\
                self._build_diff_diff_c_phi_diff_diff_matrices(t_new)
        return
    ...
    def _build_diff_diff_c_phi_matrices(self, t_new: tf.Tensor)\
            -> tf.Tensor:
        diff_diff_c_phi_matrices = self.kernel.compute_diff_diff_c_phi(t_new,
                                                                       t_new)
        return diff_diff_c_phi_matrices

    def _build_c_phi_diff_diff_matrices(self, t_new: tf.Tensor)\
            -> tf.Tensor:
        matrices = self.kernel.compute_c_phi_diff_diff(t_new, t_new)
        return matrices

    def _build_diff_c_phi_diff_diff_matrices(self, t_new: tf.Tensor)\
            -> tf.Tensor:
        matrices = self.kernel.compute_diff_c_phi_diff_diff(t_new, t_new)
        return matrices

    def _build_diff_diff_c_phi_diff_matrices(self, t_new: tf.Tensor)\
            -> tf.Tensor:
        matrices = self.kernel.compute_diff_diff_c_phi_diff(t_new, t_new)
        return matrices

    def _build_diff_diff_c_phi_diff_diff_matrices(self, t_new: tf.Tensor)\
            -> tf.Tensor:
        matrices = self.kernel.compute_diff_diff_c_phi_diff_diff(t_new, t_new)
        return matrices
    ...
    def compute_posterior_derivative_mean(self, system: tf.Tensor) -> tf.Tensor:
        """
        Compute the mean of derivative GP posterior. (Dx)
        :param system: values of the states of the system;
        :return: the TensorFlow tensor with the mean.
        """
    ...
    def compute_posterior_derivative_variance(self) -> tf.Tensor:
        """
        Compute the derivative posterior variance matrix of the training points. (A)
        :return: the TensorFlow tensor with the variance matrix.
        """
    ...
    def compute_D_prime_parts(self):
        """
        Computes s and m such that D' = s * m^-1
        :return: Tensor s and Tensor m
        """
        m_1 = tf.concat([self.c_phi_matrices_noiseless, self.c_phi_diff_matrices], 2) # first row
        m_2 = tf.concat([self.diff_c_phi_matrices, self.diff_c_phi_diff_matrices], 2) # sec row
        m = tf.concat([m_1, m_2], 1) + tf.expand_dims(tf.eye(2 * self.n_points, dtype=tf.float64), 0) \
            * self.jitter
        s = tf.concat([self.diff_diff_c_phi_matrices, self.diff_diff_c_phi_diff_matrices], 2)
        return s, m

    def compute_A_prime(self) -> tf.Tensor:
        """
        Computes A'.
        :return: Tensor of A'
        """
        s, m = self.compute_D_prime_parts()

        t = tf.concat([self.c_phi_diff_diff_matrices, self.diff_c_phi_diff_diff_matrices], 1)
        q = tf.linalg.solve(m, t)
        A_prime = self.diff_diff_c_phi_diff_diff_matrices - tf.matmul(s, q)
        return A_prime
