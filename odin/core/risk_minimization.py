"""
Implementation of the ODIN with second derivatives.
...
class ODIN(object):
    ...
    def __init__(self,
                 ...
                 gamma_prime_bounds: Union[np.array, list, Tuple] = (1e-6, 100.0),
        ...
        self.gaussian_process = GaussianProcess(self.n_states, self.n_p,
                                                self.gp_kernel, self.single_gp, self.use_sec_grads)
    ...
    def _build_variables(self) -> None:
        ...
        with tf.variable_scope('risk_main'):
            ...
            # gamma_prime
            if self.single_gp:
                self.log_gamma_prime = tf.Variable(np.log(self.initial_gamma_prime),
                                             dtype=tf.float64,
    ...
    def _build_second_derivative_mean(self) -> tf.Tensor:
        ...
        F = tf.expand_dims(grads / self.system_std_dev *
                                           self.t_std_dev, -1) # F(x, theta)

        # compute m_c
        A_inv = tf.linalg.inv(self.gaussian_process.compute_posterior_derivative_variance())
        m_c_1 = tf.expand_dims(tf.eye(self.n_points, dtype=tf.float64), 0) * tf.pow(self.gamma, -1.0) + A_inv
        m_c_2 = F / self.gamma + tf.matmul(A_inv, self.gaussian_process.compute_posterior_derivative_mean(self.x))
    ...
    def _build_second_derivative_variance(self) -> tf.Tensor:
        ...
        A_inv = tf.linalg.inv(self.gaussian_process.compute_posterior_derivative_variance())
        sigma_c_inv = tf.expand_dims(tf.eye(self.n_points, dtype=tf.float64), 0) * tf.pow(self.gamma, -1.0) + A_inv
        D_prime_2 = tf.gather(D_prime, [i for i in range(self.n_p, 2*self.n_p)], axis=2)
        q = tf.matmul(D_prime_2, tf.linalg.solve(sigma_c_inv, tf.transpose(D_prime_2, perm=[0, 2, 1])))
        post_sec_dev_var = A_prime + self.gamma_prime * tf.expand_dims(tf.eye(self.n_points,
                                                                                            dtype=tf.float64), 0)
        post_sec_dev_var += q
    ...
    def _build_second_derivative_risk_term(self):
        ...
        """
        unnormalized_states = self.x * self.system_std_dev + self.system_means
        sec_grads = self.trainable.compute_second_gradients(unnormalized_states)

        model_sec_derivatives = tf.expand_dims(sec_grads / self.system_std_dev *
                                               tf.pow(self.t_std_dev, 2), -1)
